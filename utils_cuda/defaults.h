...
#define DEFAULTS_DEBUG 0            /**< set to 1 for extra debugging info at runtime*/
...
class Defaults {
  ...
  string  inputFileName;      /**< Default input filename */
...
public:
   ...
   */
  Defaults(int argc, const char** argv, string algo);
   ...
   *         CAMPAIGN_DEFAULTS_FAIULRE upon failure.
   */
  int     setInputFileName(string valueIn);
...
private:
   ...
   */
  void    checkHasNoArguments(string param);
   ...
   * \param param parameter to check for arguments
   */
  void    checkHasArguments(string param);
   ...
   */
  void    checkArgumentHasInteger(string param);
};
