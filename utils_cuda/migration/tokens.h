...
#define TOKENS_FAILURE  1
   ...
   * string internally, until the specific type is requested for that argument.<br>
   ...
   **/
class Tokens {
  typedef multimap<string, string> paramsType;
  paramsType params;
...
public:
   ...
   **/
  int getIntegerArgument(string name, int instance);
   ...
   **/
  float   getFloatArgument(string name, int instance);
  ...
  /**
   * @brief Gets the specified parameter's specified argument as a string.
   * @param name - name of the parameter from which to get argument.
   * @param instance - if more than one argument exists for that parameter,
   *                       returns the insatnce-th argument.
   * @return the specified parameter's argument as a string.
   * If there is no argument associated with name, returns empty string.  If there is no
   * instance-th argument, returns empty string.
   **/
  string  getStringArgument(string name, int instance);

  /**
   * @brief Gets the specified parameter's first argument as an string.
   * @param name - name of the parameter to get the argument from
   * @return the specified parameter's argument as a float
   **/
   ...
   **/
  bool    hasArgument(string parameter);
   ...
   **/
  int getArgumentInstances(string name);

  /**
   * @brief Returns the argumentNumber-th argument as a string.
   * @param argument number.
   * @return Returns argumentNumber-th argument as a string.  If argumentNumber is
   *         larger than the number of elements, returns empty string.
   **/
  string  getArgument(int argumentNumber);
   ...
   **/
  bool    argumentIsNumber(string name);
   ...
   * returns false.  If name is not a parameter, returns false.
   **/
  bool    argumentIsNumber(string name, int instance);
   ...
   **/
  int addParameter(string name, string argument);
